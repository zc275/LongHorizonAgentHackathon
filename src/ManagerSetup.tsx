...
const providers: { id: ProviderId; name: string; role: string; steps: string[]; href: string; account?: string }[] = [
  ...
  { id: "rawtree", name: "RawTree", role: "Keep the context between sessions", steps: ["Use the default database, or enter a database an admin created for this demo.", "Enter a read_write API key and the selected database. Keys are cluster-wide; the app selects this database explicitly.", "Test read access, load the sample household, then read it back in Demo assets."], href: "https://rawtree.com/docs/quickstart/api", account: "https://rawtree.com" }
];
export function ConnectionsSettings({ onStates, onRequest, focus, onShoppingPreview, cameraSettings }: { onStates: (states: ConnectionStates) => void; onRequest: (provider: string | null) => void; focus: { id: string } | null; onShoppingPreview: () => void; cameraSettings: (expanded: boolean, toggle: () => void) => ReactNode }) {
  ...
  const [values, setValues] = useState<Record<string, string>>({ liquid: "http://localhost:8080/v1", database: "default" });
...
export function DemoAssets({ rawtreeReady, onRequest }: { rawtreeReady: boolean; onRequest: (provider: string | null) => void }) {
  ...
  function download() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = "nurserai-household.json"; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  return <>
    ...
    <section className="asset-section"><h3>Test your phone camera</h3><p>Open the nursery clip full screen on this computer, point your phone’s rear camera at it, and watch the manager update.</p><a className="secondary-button test-screen-link" href="/test-screen.html" target="_blank" rel="noreferrer">Open camera test screen ↗</a></section>
