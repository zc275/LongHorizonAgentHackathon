...
Tinybird is connected as an asynchronous analytics mirror for every processed observation and accepted state mutation. SQLite remains Nightwatch's source for canonical state, the append-only recovery log, and restart recovery. Tinybird delivery failures are logged and never block monitoring.
...
The API token used by Nightwatch needs `DATASOURCE:APPEND` permission for `nightwatch_state_changes`. The Events API requires an existing data source and append permission. The host must match your workspace's region.
